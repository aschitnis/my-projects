...
namespace ConsoleApp1
{
	public class DecoratorStream : Stream
	{
		...
		public DecoratorStream(Stream stream, string prefix) : base()
		{
			this.stream = stream;
			this.prefix = prefix;
			byte[] bPrefix = Encoding.UTF8.GetBytes(this.prefix);

			Write(Encoding.UTF8.GetBytes(this.prefix), 0, bPrefix.Length);
		}
		...
		{
			int numberOfbytesRead = stream.Read(bytes, offset, count);
